...
def test_sqlalchemy_version():
    ...
    assert (major, minor) >= (2, 0), f"SQLAlchemy version {version} is too old, need 2.0+"
...
async def test_plugin_fixtures_with_different_versions(sqlite_engine):
    ...
    from pytest_green_light.fixtures import async_session_factory, async_db_transaction
    from sqlalchemy.ext.asyncio import AsyncSession, async_sessionmaker
